...
func (f *fakePodWorkers) UpdatePod(options kubelet.UpdatePodOptions) {
	f.lock.Lock()
	defer f.lock.Unlock()
	var uid types.UID
	switch {
...
func (f *fakePodWorkers) SyncKnownPods(desiredPods []*v1.Pod) map[types.UID]kubelet.PodWorkerSync {
	return map[types.UID]kubelet.PodWorkerSync{}
}
...
func (w *timeIncrementingWorkers) UpdatePod(options kubelet.UpdatePodOptions, afterFns ...func()) {
	func() {
		w.lock.Lock()
		defer w.lock.Unlock()
		w.UpdatePod(options)
		(w.w.GetClock()).(*clocktesting.FakePassiveClock).SetTime((w.w.GetClock()).Now().Add(time.Second))
...
func (w *timeIncrementingWorkers) SyncKnownPods(desiredPods []*v1.Pod) (knownPods map[types.UID]kubelet.PodWorkerSync) {
	func() {
		w.lock.Lock()
		defer w.lock.Unlock()
		knownPods = w.SyncKnownPods(desiredPods)
		(w.w.GetClock()).(*clocktesting.FakePassiveClock).SetTime((w.w.GetClock()).Now().Add(time.Second))
	}()
	w.drainUnpausedWorkers()
	return
...
func (w *timeIncrementingWorkers) drainUnpausedWorkers() {
	...
	for {
		...
		(w.w.GetPodLock()).Lock()
		for uid, worker := range w.w.GetPodSyncStatuses() {
...
// 		runtime: runtime,
// 	}
// 	nestedEx.workerChannelFn = func(uid types.UID, in chan struct{}) <-chan struct{} {
// 		ch := make(chan struct{})
// 		go func() {
// 			defer close(ch)
...
// func createPodWorkers() (*kubelet.PodWorkers, *containertest.FakeRuntime, map[types.UID][]syncPodRecord) {
// 	lock := sync.Mutex{}
...
// 	w := kubelet.NewPodWorkers(
// 		kubelet.PodSyncerFuncs{
...
// func drainWorkers(podWorkers *kubelet.PodWorkers, numPods int) {
// 	for {
// 		stillWorking := false
...
// func drainWorkersExcept(podWorkers *kubelet.PodWorkers, uids ...types.UID) {
// 	set := sets.NewString()
// 	for _, uid := range uids {
...
// func drainAllWorkers(podWorkers *kubelet.PodWorkers) {
// 	for {
// 		stillWorking := false
// 		podWorkers.podLock.Lock()
...
func newTerminalPhaseSync(fn kubelet.SyncPodFnType) *terminalPhaseSync {
